...
export type TGetTopicAction = ITopicBeginAction | ITopicSuccessAction | ITopicFailureAction;

const createGetTopicBegin: ActionCreator<ITopicBeginAction> = () => ({
    type: GET_TOPIC_ENUM.REQUEST,
})

const createGetTopicSuccess: ActionCreator<ITopicSuccessAction> = (payload: ITopicBaseData) => ({
    type: GET_TOPIC_ENUM.SUCCESS,
    payload
})
